...
BOOL yq_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void yq_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void yq_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
@implementation NSObject (Runtime_yq)

+ (void)yq_swizzleMethod:(SEL)originalMethod withMethod:(SEL)newMethod
{
    yq_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)yq_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    yq_method_append(self.class, klass, newMethod);
}

+ (void)yq_replaceMethod:(SEL)method fromClass:(Class)klass
{
    yq_method_replace(self.class, klass, method);
}

- (BOOL)yq_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class yq_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)yq_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)yq_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass yq_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)yq_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{

...
@implementation NSString (Emoji_yq)

static NSDictionary * yq_s_unicodeToCheatCodes = nil;
static NSDictionary * yq_s_cheatCodesToUnicode = nil;

+ (void)yq_initializeEmojiCheatCodes
{
    ...
    @synchronized(self) {
        yq_s_unicodeToCheatCodes = forwardMap;
        yq_s_cheatCodesToUnicode = [reversedMap copy];
    }
}

- (NSString *)yq_stringByReplacingEmojiCheatCodesWithUnicode
{
    if (!yq_s_cheatCodesToUnicode) {
        [NSString yq_initializeEmojiCheatCodes];
    }
    
    if ([self rangeOfString:@":"].location != NSNotFound) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [yq_s_cheatCodesToUnicode enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL *stop) {
...
- (NSString *)yq_stringByReplacingEmojiUnicodeWithCheatCodes
{
    ...
    if (self.length) {
        __block NSMutableString *newText = [NSMutableString stringWithString:self];
        [yq_s_unicodeToCheatCodes enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {

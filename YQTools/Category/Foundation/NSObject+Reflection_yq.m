...
@implementation NSObject (Reflection_yq)
...
{
    return [[self class] yq_propertyKeys];
}
+ (NSArray *)yq_propertyKeys {
...
{
    return [[self class] yq_propertiesInfo];
}
/**
...
+ (NSArray *)yq_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self yq_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)yq_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] yq_propertiesInfo];
...
-(NSArray*)yq_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] yq_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] yq_decodeType:returnType];
        NSString *encodeString = [[self class] yq_decodeType:encoding];
...
-(NSDictionary *)yq_protocolList{
    return [[self class]yq_protocolList];
}
+ (NSDictionary *)yq_protocolList
{
...
+ (NSArray *)yq_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] yq_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)yq_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)yq_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString yq_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
